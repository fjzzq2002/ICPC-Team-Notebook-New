/**
 * Author: Claris
 * Description: Toptree. Think very carefully before writing this beast!!
 */
...
//son: 0-1 successor in chain; 2-3 AAA child
...
void add(int x,int y){//vlink x->y
  if(!y)return;
  pb(x);
...
void del(int x){//remove vlink of x and parent
  if(!x)return;
  splay(x);
  if(!f[x])return;
  int y=f[x];
...
int fa(int x){//x father through vlinks
  splay(x);
  if(!f[x])return 0;
...
int main(){
  ...
  while(m--){
    ...
    if(k==0){//set vals in x subtree to y
      read(x),read(y);
      changetree(x,tag(0,y));
    }
    if(k==5){//add y to vals in x subtree
      read(x),read(y);
      changetree(x,tag(1,y));
    }
    if(k==3){//minimum in x subtree
      read(x);
      printf("%d\n",asktree(x).minv);
    }
    if(k==4){//maximum in x subtree
      read(x);
      printf("%d\n",asktree(x).maxv);
    }
    if(k==11){//sum in x subtree
      read(x);
      printf("%d\n",asktree(x).sum);
    }
    if(k==2){//set vals in chain
      read(x),read(y),read(z);
      changechain(x,y,tag(0,z));
      makeroot(rt);
    }
    if(k==6){//add to vals in chain
      read(x),read(y),read(z);
      changechain(x,y,tag(1,z));
      makeroot(rt);
    }
    if(k==7){//minimum in chain
      read(x),read(y);
      printf("%d\n",askchain(x,y).minv);
      makeroot(rt);
    }
    if(k==8){//maximum in chain
      read(x),read(y);
      printf("%d\n",askchain(x,y).maxv);
      makeroot(rt);
    }
    if(k==10){//sum in chain
      read(x),read(y);
      printf("%d\n",askchain(x,y).sum);
      makeroot(rt);
    }
